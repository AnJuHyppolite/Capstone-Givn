...
itemsForUser.post('/', async(req,res)=> req.body.title && req.body.description && req.body.location && req.body.status ? res.json(await createItem(req.params.user_id, req.body)) : res.status(422).json({success: false, error: true, message: "title is required"}))
